...
router.route('/').get(getAllSub_Category).post(verifyToken, Sub_CategoryImageUpload.fields([{ name: 'sub_category_image', maxCount: 1 }]), postSub_Category).patch(verifyToken, Sub_CategoryImageUpload.fields([{ name: 'sub_category_image', maxCount: 1 }]), updateSub_CategoryInfo).delete(verifyToken, deleteASub_CategoryInfo)
